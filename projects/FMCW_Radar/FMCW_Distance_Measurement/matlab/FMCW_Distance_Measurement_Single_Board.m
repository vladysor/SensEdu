...
Tc = 0.04;              % Duration of one chirp (s)
c = 343;                 % Speed of sound in air for T=300K (m/s)
d_cor = -0.018;            % Distance correction (m)   
...
for it = 1:ITERATIONS
    ...
    mixed_signal_filt = lowpass(mixed_signal, 6000, SAMPLING_RATE);
    mixed_signal_filt = highpass(mixed_signal_filt, 400, SAMPLING_RATE);
    ...
    % Exclude the first 15 samples from the signal
    p_mix_filt_trimmed = p_mix_filt(25:end);  % Skip the first 25 samples
    f_mix_filt_trimmed = f_mix_filt(25:end);  % Synchronize the corresponding frequencies

    % Find the peak in the trimmed signal
    [p_fbeat, fbeat] = findpeaks(p_mix_filt_trimmed, f_mix_filt_trimmed, ...
    ...
    d = (fbeat * Tc * c) / (2*(f_end - f_start))+d_cor;
