...
const uint8_t pwm_pin = D4;
const uint8_t pwm_mask_pin = D5;
...
void setup() {
    ...
    pinMode(pwm_mask_pin, OUTPUT);
    digitalWrite(pwm_mask_pin, 0);
...
void loop() {
    ...
    switch(status) {
        case CMD_STATUS_INVALID_INPUT:
            ...
            Serial.println("Expected input: [fstart, fend, steps, step_dur_ms, pause_ms]");
            break;
...
static void pwm_sequence(uint32_t* params) {
    ...
    for (uint32_t i = 0; i < steps; i++) {
        ...
        digitalWrite(pwm_mask_pin, 1);
        SensEdu_PWM_Start();
        SensEdu_TIMER_Delay_us(1000 * step_dur);
        SensEdu_PWM_Stop();
        digitalWrite(pwm_mask_pin, 0);
        SensEdu_TIMER_Delay_us(1000 * pause_ms);
    }
}
...
static void print_params(uint32_t* params) {
    char buf[128];
    sprintf(
        buf, "Applied params: fstart = %luHz, fend = %luHz, steps = %lu, step_dur = %lums, pause = %lums",
